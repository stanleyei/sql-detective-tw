...
SD.chapters.push({
  ...
  steps: [
    { type: 'story', lines: [
      ...
      { who: 'vendor', text: '警察先生！我轉身拿貨才幾秒，回頭展示盒就被打開了，五副耳機全沒了！有個穿黑外套的男生跳上白色機車跑掉，車牌開頭是 MKJ！' },
...
WHERE id IN (10001, 10002)</code></pre>` },
    { type: 'task', id: 'c1-t3', title: '縮小範圍', prompt: '在上一題的基礎上，只留下<strong>白色</strong>、<strong>機車</strong>、身高 <strong>175 到 180</strong> 公分的駕照，顯示 <code>id</code>、<code>plate_number</code>、<code>person_id</code>。',
...
LIMIT 3;</code></pre>
    ...
    { type: 'task', id: 'c1-t8', title: '嫌疑人筆錄', prompt: '從 <code>interview</code>（筆錄）找出 <code>person_id</code> 為 <strong>4 或 5</strong> 的筆錄，顯示 <code>person_id</code>、<code>transcript</code>，並依 <code>person_id</code> 排序。',
      hints: ['IN (4, 5)', 'ORDER BY person_id', '<code>SELECT person_id, transcript FROM interview WHERE person_id IN (4, 5) ORDER BY person_id;</code>'],
    ...
    { type: 'task', id: 'c1-t10', title: '打卡紀錄', prompt: '用上一題查到的會員編號（<code>membership_id</code>），在 <code>gym_checkin</code> 找出 <strong>2025-03-08</strong> 的打卡，顯示 <code>checkin_time</code> 與 <code>checkout_time</code>。',
