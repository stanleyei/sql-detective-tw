...
DATEDIFF(d1, d2)               -- 兩個日期差幾天</code></pre>
    ...
    { type: 'task', id: 'c2-t4', title: '案發前後的監視器', prompt: '從 <code>cctv_log</code> 找出 <code>camera_location</code> 為<strong>海濱門市前</strong>、時間在 <strong>2025-04-12 22:30:00 到 23:10:00</strong> 之間的紀錄，顯示 <code>capture_time</code>、<code>plate_number</code>、<code>note</code>，依時間排序。',
      hints: ['地點用 =，時間用 BETWEEN。', 'ORDER BY capture_time', "<code>SELECT capture_time, plate_number, note FROM cctv_log WHERE camera_location = '海濱門市前' AND capture_time BETWEEN '2025-04-12 22:30:00' AND '2025-04-12 23:10:00' ORDER BY capture_time;</code>"],
...
UPPER(x)  LOWER(x)  TRIM(x) -- 大寫、小寫、去頭尾空白</code></pre>
    ...
    { type: 'task', id: 'c2-t6', title: '銀色、女性', prompt: '同上，但只留下<strong>銀色</strong>汽車且駕駛為<strong>女性</strong>的駕照，顯示 <code>plate_number</code> 與 <code>person_id</code>。',
    ...
    { type: 'task', id: 'c2-t7', title: '交叉比對時間', prompt: '這兩張車牌，哪一張在 <strong>22:30 到 23:10</strong> 之間出現在 <code>cctv_log</code>？顯示<strong>不重複</strong>的 <code>plate_number</code>。',
...
FROM cctv_log;</code></pre>
    ...
    { type: 'task', id: 'c2-t10', title: '她的車出現在什麼時段？', prompt: '列出 <code>cctv_log</code> 中車牌 <strong>RBK-7528</strong> 的所有紀錄，顯示 <code>capture_time</code>，並用 CASE 依小時標記 <code>period</code>（6~17 白天、18~23 晚上、其他深夜），依時間排序。',
    ...
    { type: 'task', id: 'c2-t11', title: '格式化時間', prompt: '同樣是 RBK-7528 的紀錄，用 <code>DATE_FORMAT(capture_time, \'%m/%d %H:%i\')</code> 顯示成「月/日 時:分」，別名 <code>t</code>，並把 <code>camera_location</code> 一起列出，依時間排序。',
