...
WHERE district = '港東區';</code></pre>
    ...
    { type: 'task', id: 'c0-t5b', title: '中央區以外的人', prompt: '列出<strong>不住在中央區</strong>的市民姓名（<code>name</code>）與行政區（<code>district</code>）。',
      ...
      check: { kind: 'result', cols: ['name', 'district'], ordered: false } },
    { type: 'task', id: 'c0-d1', variant: 'debug', title: '學長留下的 SQL', prompt: '學長想列出<strong>港西區</strong>居民的 <code>name</code>，但這句 SQL 一執行就報錯。找出並修正錯誤，讓它跑出結果。',
      starter: 'SELECT name, FROM person WHERE district = 港西區;',
      hints: ['看錯誤訊息：FROM 前面多了一個逗號。改完再執行，會出現第二個錯。', '文字值要用單引號包起來：\'港西區\'。', '<code>SELECT name FROM person WHERE district = \'港西區\';</code>'],
      check: { kind: 'result', cols: ['name'], ordered: false } },
    ...
    { type: 'predict', id: 'c0-p1', title: 'LIMIT 會怎麼截？', sql: "SELECT name FROM person WHERE birth_year > 2000 LIMIT 3;", question: '這句會回傳幾筆？', options: ['0 筆', '3 筆', '所有 2000 年後出生的人', '會出錯，WHERE 和 LIMIT 不能同時用'], answer: 1, explain: 'WHERE 先篩出 2000 年後出生的人，LIMIT 再從結果裡只留前 3 筆。兩者可以同時用，順序固定是 WHERE 在前、LIMIT 在後。' },
